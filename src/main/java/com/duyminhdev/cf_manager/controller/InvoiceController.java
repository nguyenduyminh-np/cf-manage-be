...
public class InvoiceController {
    ...
    public ApiResponse<PageResponse<List<InvoiceListItemDTO>>> search(
        ...
        return new ApiResponse<>(200, "SUCCESS", invoiceService.search(request));
    }

    @PostMapping("/export")
    public void exportExcel(@Valid @RequestBody InvoiceSearchRequestDTO request,
                            HttpServletResponse response) throws IOException {
        List<InvoiceExportDTO> items = invoiceService.exportData(request);

        String fileName = "DANH_SACH_HOA_DON_" + System.currentTimeMillis() + ".xlsx";
        String title = "DANH SÁCH HÓA ĐƠN";

        ExcelUtils.export(response, InvoiceExportDTO.class, items, fileName, title);
    }
