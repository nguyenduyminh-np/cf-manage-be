...
> **Phiên bản:** 3.2 — Synced with implementation  
> **Cập nhật:** 2026-04-22  
...
>
> **⚠️ LƯU Ý QUAN TRỌNG (v3.2):**
> 1. **Kiểu thời gian:** Toàn bộ field thời gian dùng `Instant` (UTC), KHÔNG phải `LocalDateTime`. FE gửi/nhận dạng ISO-8601 UTC: `"2026-04-10T12:00:00.000Z"`.
> 2. **API routing:** Tất cả action endpoints dùng **POST với `bookingId` trong request body**, KHÔNG dùng path variable `/{id}/action`.
> 3. **Available Slots:** Endpoint là `POST /tables/available-slots` với request body, KHÔNG phải `GET` với query params.
...
- [A13. Xem chi tiết đơn đặt bàn (Detail Booking)](#a13-xem-chi-tiết-đơn-đặt-bàn-detail-booking)
- [A14. Lấy danh sách booking Pending/Confirmed (Pending Job)](#a14-lấy-danh-sách-booking-pendingconfirmed-pending-job)
- [A15. Xóa đơn đặt bàn (Delete Booking)](#a15-xóa-đơn-đặt-bàn-delete-booking)
...
{
  "tableId": 8,
  "expectedArriveTime": "2026-04-10T12:00:00.000Z",
  "expectedCheckOut": "2026-04-10T14:00:00.000Z",
...
| `expectedArriveTime` | `Instant`    | ✅ **Có** | `@NotNull`. Alias: `bookingTime`              | Thời gian khách dự kiến đến (UTC). BE enforce thêm: ≥ NOW() + 2h và trong khung 06:00 - 20:00 (giờ quán, UTC+7) |
| `expectedCheckOut`   | `Instant`    | ✅ **Có** | `@NotNull`                                    | Thời gian dự kiến rời (UTC). FE mặc định = arriveTime + 2h                                                |
...
| `depositPaidAt`      | `Instant`    | Không     | —                                             | Thời gian thanh toán cọc (UTC)                                                                             |
...
```
POST /api/v1/table-booking/confirm
```
...
### Request — `TableBookingConfirmRequestDTO`

> **Source:** `dto/request/table_booking/TableBookingConfirmRequestDTO.java`
...
| Field       | Type      | Bắt buộc  | Validation              | Mô tả                |
| ----------- | --------- | --------- | ----------------------- | -------------------- |
| `bookingId` | `Integer` | ✅ **Có** | `@NotNull`, `@Positive` | ID booking cần confirm |
...
```
POST /api/v1/table-booking/check-in
```
...
{
  "bookingId": 123,
  "checkInAt": "2026-04-10T10:50:00.000Z",
  "force": false
}
```
...
| Field       | Type      | Bắt buộc  | Mô tả                                                                          |
| ----------- | --------- | --------- | ------------------------------------------------------------------------------ |
| `bookingId` | `Integer` | ✅ **Có** | ID booking cần check-in. `@NotNull`, `@Positive`                               |
| `checkInAt` | `Instant` | Không     | Thời gian check-in thực tế (UTC). Nếu null → BE dùng `Instant.now()`            |
...
```
POST /api/v1/table-booking/check-out
```
...
> **Source:** `dto/request/table_booking/TableBookingCheckOutRequestDTO.java`
...
{
  "bookingId": 123,
  "checkOutAt": "2026-04-10T13:05:00.000Z"
}
```

| Field        | Type      | Bắt buộc  | Mô tả                                                          |
| ------------ | --------- | --------- | -------------------------------------------------------------- |
| `bookingId`  | `Integer` | ✅ **Có** | ID booking cần check-out. `@NotNull`, `@Positive`               |
| `checkOutAt` | `Instant` | Không     | Thời gian checkout thực tế (UTC). Nếu null → BE dùng `Instant.now()` |
...
```
POST /api/v1/table-booking/cancel
```
...
### Request — `TableBookingCancelRequestDTO`

> **Source:** `dto/request/table_booking/TableBookingCancelRequestDTO.java`
...
| Field       | Type      | Bắt buộc  | Validation              | Mô tả               |
| ----------- | --------- | --------- | ----------------------- | ------------------- |
| `bookingId` | `Integer` | ✅ **Có** | `@NotNull`, `@Positive` | ID booking cần hủy |
...
```
POST /api/v1/table-booking/deposit
```
...
```json
{
  ...
  "depositPaidAt": "2026-04-10T07:30:00.000Z",
  "depositTxnRef": "MOMO-20260410-001"
}
```
...
| Field           | Type         | Bắt buộc  | Validation                                     | Mô tả                                                      |
| --------------- | ------------ | --------- | ---------------------------------------------- | ---------------------------------------------------------- |
| `bookingId`     | `Integer`    | ✅ **Có** | `@NotNull`, `@Positive`                        | ID booking cần ghi nhận cọc                                 |
| `depositAmount` | `BigDecimal` | Có\*      | `@DecimalMin("0", inclusive=false)` → phải > 0 | Số tiền cọc                                                  |
| `depositPaidAt` | `Instant`    | Không     | —                                              | Thời gian thanh toán (UTC). Nếu null → BE dùng `Instant.now()` |
...
```
POST /api/v1/table-booking/extend
```
...
{
  "bookingId": 123,
  "expectedCheckOut": "2026-04-10T15:00:00.000Z",
  "force": false
}
```
...
| Field              | Type      | Bắt buộc  | Validation              | Mô tả                                                 |
| ------------------ | --------- | --------- | ----------------------- | ----------------------------------------------------- |
| `bookingId`        | `Integer` | ✅ **Có** | `@NotNull`, `@Positive` | ID booking cần gia hạn                                  |
| `expectedCheckOut` | `Instant` | ✅ **Có** | `@NotNull`              | Thời gian checkout mới (UTC, phải xa hơn giá trị hiện tại) |
...
| `expectedArriveTime` | `Instant` | Không    | —                        | BE auto-set = `Instant.now()` cho walk-in |
| `expectedCheckOut`   | `Instant` | Không    | —                        | BE auto-set = `Instant.now()` + 2h       |
...
| `bookingStatus` | `String`  | Không    | Lọc theo trạng thái                                        |
| `customerName`  | `String`  | Không    | Tìm kiếm LIKE theo tên khách                              |
| `phoneNumber`   | `String`  | Không    | Tìm theo SĐT                                              |
| `checkInAt`     | `Instant` | Không    | Lọc theo thời gian check-in (JSON alias: `check_in_at`)   |
| `checkOutAt`    | `Instant` | Không    | Lọc theo thời gian check-out (JSON alias: `check_out_at`) |
...
| `expectedArriveTime` | `Instant`    | Giờ hẹn đến (UTC, format `yyyy-MM-dd'T'HH:mm:ss.SSSX`)                                                 |
| `checkInAt`          | `Instant`    | Giờ check-in thực tế (null nếu chưa)                                                                    |
| `expectedCheckOut`   | `Instant`    | Giờ dự kiến rời                                                                                         |
| `checkOutAt`         | `Instant`    | Giờ check-out thực tế (null nếu chưa)                                                                   |
...
| `depositPaidAt`      | `Instant`    | Thời gian cọc (UTC)                                                                                     |
...
| `createdAt`          | `Instant`    | Thời gian tạo (UTC)                                                                                     |
...
```
POST /api/v1/table-booking/tables/available-slots
```

> ⚠️ **Thay đổi so với thiết kế ban đầu:** Endpoint thực tế là `POST` với request body, KHÔNG phải `GET` với path variable và query param.
...
### Request — `TableBookingAvailableSlotsRequestDTO`

> **Source:** `dto/request/table_booking/TableBookingAvailableSlotsRequestDTO.java`
...
| Field     | Type        | Bắt buộc  | Validation              | Mô tả                                    |
| --------- | ----------- | --------- | ----------------------- | ---------------------------------------- |
| `tableId` | `Integer`   | ✅ **Có** | `@NotNull`, `@Positive` | ID bàn cần xem khung giờ trống           |
| `date`    | `LocalDate` | ✅ **Có** | `@NotNull`              | Ngày cần xem (format `yyyy-MM-dd`)       |
...
```json
{
  ...
  "data": [
    {
      "slotStart": "2026-04-10T01:00:00.000Z",
      "slotEnd": "2026-04-10T04:00:00.000Z"
    },
    {
      "slotStart": "2026-04-10T06:00:00.000Z",
...
| Field       | Type      | Mô tả                       |
| ----------- | --------- | --------------------------- |
| `slotStart` | `Instant` | Đầu khung giờ trống (UTC)   |
| `slotEnd`   | `Instant` | Cuối khung giờ trống (UTC)  |
...
| `expectedArriveTime` | `Instant` | ✅ **Có** | `@NotNull`. Alias: `bookingTime` |
| `expectedCheckOut`   | `Instant` | ✅ **Có** | `@NotNull`                       |
...
> **FE mới nên migrate sang action endpoints riêng** (`/confirm`, `/check-in`, `/check-out`, `/cancel`, `/deposit`, `/extend`) thay vì dùng endpoint này cho thay đổi trạng thái. Endpoint `/update` chỉ nên dùng cho thay đổi **thông tin phi trạng thái** (đổi bàn, đổi giờ, sửa tên/SĐT).
> **Lưu ý:** Cc action endpoints dùng body-based (`bookingId` trong body), KHÔNG dùng path variable.
...
| `checkInAt`     | `Instant` | Không     | —                         |
| `checkOutAt`    | `Instant` | Không     | —                         |
...
## A13. Xem chi tiết đơn đặt bàn (Detail Booking)

### Endpoint

```
POST /api/v1/table-booking/detail
```

> **API MỚI** — Chưa có trong tài liệu thiết kế ban đầu.

### Mục đích

Lấy thông tin chi tiết của **một booking** theo `bookingId`.

### Request — `TableBookingDetailRequestDTO`

> **Source:** `dto/request/table_booking/TableBookingDetailRequestDTO.java`
...
| Field       | Type      | Bắt buộc  | Validation              | Mô tả                     |
| ----------- | --------- | --------- | ----------------------- | ------------------------- |
| `bookingId` | `Integer` | ✅ **Có** | `@NotNull`, `@Positive` | ID booking cần xem chi tiết |
...
## A14. Lấy danh sách booking Pending/Confirmed (Pending Job)

### Endpoint

```
POST /api/v1/table-booking/pending-job
```
...
Lấy danh sách các booking đang ở trạng thái `PENDING_CONFIRMATION` hoặc `CONFIRMED` cần xử lý. Dùng cho màn hình quản lý đơn chờ.

### Request — `TableBookingSearchRequestDTO`

Cùng DTO với Search Booking (A9). Hỗ trợ phân trang.
...
```json
{
  ...
  "data": {
    "content": [ "...TableBookingResponseDTO..." ],
    "totalElements": 15,
    "totalPages": 2,
    "number": 0,
    "size": 10
...
## A15. Xóa đơn đặt bàn (Delete Booking)

### Endpoint

```
POST /api/v1/table-booking/delete
```
...
Xóa mềm (soft delete) một booking. Set `active = false`.

### Request

- **Body:** Dùng cùng DTO với Confirm (`TableBookingConfirmRequestDTO`)
...
| Field       | Type      | Bắt buộc  | Validation              | Mô tả               |
| ----------- | --------- | --------- | ----------------------- | ------------------- |
| `bookingId` | `Integer` | ✅ **Có** | `@NotNull`, `@Positive` | ID booking cần xóa  |
...
{
  "status": 200,
  "message": "DELETE_TABLE_BOOKING_SUCCESS",
  "data": true
}
```
...
| elapsed ≥ 20p         | Auto `CANCELLED`, bàn `AVAILABLE`, hoàn cọc | `NO_ORDER_AUTO_CANCELLED` via `/topic/table-status` |
...
| `POST /confirm`                  | `CONFIRM_TABLE_BOOKING_SUCCESS`                                             |
| `POST /check-in`                 | `CHECK_IN_TABLE_BOOKING_SUCCESS`                                            |
| `POST /check-out`                | `CHECK_OUT_TABLE_BOOKING_SUCCESS`                                           |
| `POST /cancel`                   | `CANCEL_TABLE_BOOKING_SUCCESS`                                              |
| `POST /deposit`                  | `DEPOSIT_TABLE_BOOKING_SUCCESS`                                             |
| `POST /extend`                   | `EXTEND_TABLE_BOOKING_SUCCESS`                                              |
| `POST /walk-in` (legacy)         | `CREATE_WALK_IN_BOOKING_SUCCESS` hoặc `CREATE_LATE_ARRIVAL_WALK_IN_SUCCESS` |
| `POST /tables/available-slots`   | `GET_AVAILABLE_SLOTS_SUCCESS`                                               |
| `POST /detail`                   | `GET_TABLE_BOOKING_DETAIL_SUCCESS`                                          |
| `POST /pending-job`              | `GET_PENDING_JOB_SUCCESS`                                                   |
| `POST /delete`                   | `DELETE_TABLE_BOOKING_SUCCESS`                                              |
...
### Message Prefix Convention (cập nhật theo `BookingStateMachineConstant`)

> **Source:** `constant/BookingStateMachineConstant.java`

| Prefix thực tế trong code                   | Ý nghĩa                                                     | Ví dụ                                                                                                 |
| ------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `BOOKING_TRANSITION_RULE_VIOLATION: `       | Rule nghiệp vụ vi phạm (conflict, quá hạn, chưa cọc...)     | `BOOKING_TRANSITION_RULE_VIOLATION: [RULE_01_ADVANCE_BOOKING][AdvanceBookingValidator] must book ≥ 2h`   |
| `BOOKING_TRANSITION_NOT_ALLOWED: `          | Transition không hợp lệ trong state machine                 | `BOOKING_TRANSITION_NOT_ALLOWED: [STATE_MACHINE_TRANSITION][BookingStateMachineImpl] PENDING → COMPLETED` |
| `BOOKING_TRANSITION_GUARD_FAILED: `         | Thiếu dữ liệu bắt buộc (booking null, table null, inactive) | `BOOKING_TRANSITION_GUARD_FAILED: [STATE_MACHINE_GUARD][BookingStateMachineImpl] booking is inactive`    |

> ⚠️ **Lưu ý:** Tài liệu thiết kế ban đầu dùng `[RULE_VIOLATION]`, `[NOT_ALLOWED]`, `[GUARD_FAILED]`. Code thực tế dùng **full prefix** như trên. FE cần parse theo `startsWith()` để phân loại lỗi.
...
PENDING_CONFIRMATION  →  EXPIRED                 (đơn chờ xác nhận quá lâu — NEW trong code thực tế)
